...
public class ConsoleEditor implements Editor {
    ...
    public void run(String text) {
        ...
        while (!exit) {
            ...
            if(text.equals("text")){
                showDocFormat(documentLines);
            } else {
                showJsonFormat(documentLines);
            }
            showHelp();
        }
    }

    private void showDocFormat(ArrayList<String> textLines) {
        if (textLines.size() > 0){
    ...
    private void showJsonFormat(ArrayList<String> textLines) {
        if (textLines.size() > 0){
            setTextColor(TEXT_YELLOW);
            printLnToConsole("START DOCUMENT ==>");
            printLnToConsole("{");
